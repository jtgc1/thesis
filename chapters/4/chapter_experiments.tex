...
A representative sample is shown in Figure~\ref{fig:realMeas_intro}, illustrating the range of behaviours observed across stake locations. Some locations exhibit a clear decline in PHC concentration consistent with active biostimulation, while others show more complex dynamics including periods of stagnation or increases, likely attributable to soil heterogeneity or localised variations in nutrient distribution. Finally, the real-world data sample in Figure~\ref{fig:realMeas_intro} is broken down into the inputs that the methods use to produce a spatial estimate of the plume and the resulting truth value that the methods are evaluated on demonstrated in Figure~\ref{fig:realMeas_breakdown}. 

 \begin{figure}[H]
 \centering
 \includegraphics[height = 6in]{figures/real_data_sample.png}
 \caption{How the method input and output data are extracted from a real data sample. (a) PHC concentration through time at three sensor depths. (b) One sensor is removed from the input. Each depth is removed once to create three inputs per data sample. (c) Single measurement point which is the $\nSampTime$ measurement from the removed sensor depth. Used as the ground truth the methods are evaluated against. }
