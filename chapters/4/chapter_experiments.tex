...
At each depth, a Beer-Lambert sensor measures absorbed light through the air to determine PHC concentration at 30-minute intervals~\cite{beerLambertSensor}. To reduce measurement noise while preserving the underlying remediation dynamics, these raw readings are aggregated into 12-hour averages, yielding two observations per depth per day at each stake location. Each data sample therefore consists of a time series of PHC concentration measurements at three depths from a single stake location, sampled at 12-hour increments, as shown in Fig. \ref{fig:realMeas_intro}.
 ...
 \begin{figure}[H]
 \centering
 \includegraphics[height = 6in]{figures/real_measurements.png}
...
The duration of the time series varies considerably across samples, ranging from 30 to 250 days. This variability reflects the practical realities of field deployment. Sensors were installed and occasionally decommissioned at different times throughout the remediation process, and some units experienced extended periods of dropout due to hardware faults or adverse weather conditions. As a result, the dataset is irregular with stake locations distributed non-uniformly across the site and each location contributing multiple different lengths of observational records. 

A representative sample is shown in Figure~\ref{fig:realMeas_intro}, illustrating the range of behaviours observed across stake locations. Some locations exhibit a clear decline in PHC concentration consistent with active biostimulation, while others show more complex dynamics including periods of stagnation or increases, likely attributable to soil heterogeneity or localised variations in nutrient distribution. The real-world data sample in Figure~\ref{fig:realMeas_intro} is broken down into the inputs and outputs that the methods use, demonstrated in Figure~\ref{fig:realMeas_breakdown}.

\begin{figure}[h!]

\centering
\includegraphics[height = 7in]{figures/real_data_sample.png}
\caption{How the method input and output data are extracted from a real data sample. (a) PHC concentration hrough time at three sensor depths. (b) One sensor is removed from the input. Each depth is removed once to create three inputs per data sample. (c) Single measurement point which is the $\nSampTime$ measurement from the removed sensor depth. Used as the ground truth the methods are evaluated against. }
\label{fig:realMeas_breakdown}
\end{figure}
...
\noindent
where $\conc(\locvar_i,\,\timevar)$ is the PHC concentration (\%) at depth $\locvar_i$~(m) and time $\timevar$~(hours), $\advecCoeff(\locvar_i)$ is a simplified lateral advection term ranging from $\bigl(-10^{-3},\;10^{-3}\bigr)$~\%\,hour$^{-1}$ \cite{AdvectionCoeff}, $\biodegCoeff$ is a proportional biodegradation rate representing the natural microbial breakdown of PHC in the soil, ranging from $\bigl(-10^{-3},\;10^{-3}\bigr)$~hour$^{-1}$ \cite{biodeg} and $\diffCoeff$ is the vertical diffusion coefficient governing PHC transport between neighbouring cells, ranging from $\bigl(10^{-10},\;10^{-6}\bigr)$~m$^{2}$\,second$^{-1}$ \cite{diffcoeff, tempCoeff}. All three soil parameters are realized as vectors with one value per simulation cell. $\npp$ (number of pilot points) values are drawn from the distributions shown in Figure~\ref{fig:sim_param_dist} and assigned to uniform all intermediate cell values are obtained by interpolation.


\subsection{Soil Parameterization}
\label{sec:synth_soil_params}

Four boundary condition scenarios are used to generate the simulated dataset. A PHC source at the top only, the bottom only, both the top and bottom of the soil column and no sources. These cases represent the real-world configurations in which PHC originates from the surface, the water table, both, or having no direct source \cite{hydrocarbon_in_soil_old,PHC_in_soil_newer}. Examples of each source location are available in Figure~\ref{fig:sim_example_plots}. Source boundary concentrations are drawn from a normal distribution with a mean of 3\% and a standard deviation of 0.5\%; all other initial PHC concentrations are drawn with a mean of 0.12\% and a standard deviation of 0.04\%, with a lower threshold of 0\%. These values are derived from the real-world data in Section~\ref{sec:real_data}. As with the soil parameters, values are assigned at pilot-point depths and interpolated across all remaining cells.

\begin{figure}[!h]
    \centering
    \includegraphics[height=5in]{figures/sim_example_combined.png}
    \caption{Sample synthetic data samples with the four respective source locations. Top, bottom, both, neither source locations representing a surface, water-table, both surface and water table and no source circumstances.}
    \label{fig:sim_example_plots}
\end{figure}
...
The boundary and initial conditions of PHC concentration are straightforward. All values are mapped linearly from their physical range of $[0, 4]$ to $[0, 1]$. The regularization of the boundary conditions and the initial conditions of PHC are quite straightforward. All of them are transformed linearly from 0-4 to 0-1. The diffusion coefficient requires a different treatment because it spans four orders of magnitude. The diffusion parameter $\diffCoeff \in [10^{-10},\, 10^{-6}]$~m$^2$/s is mapped to a normalised variable $\tilde{\diffCoeff} \in [0, 1]$ via a log-linear transformation \cite{jeffreys1946invariant}. The forward transformation from $\tilde{\diffCoeff}$ to $\diffCoeff$ is
...
This normalization ensures that equal increments in $\tilde{\diffCoeff}$ correspond to equal multiplicative steps in $\diffCoeff$, preserving sensitivity across the full range of the parameter space; first used for groundwater in \cite{freeze1975stochastic}.

The advection coefficient $\advecCoeff$ and biodegradation coefficient $\biodegCoeff$ are signed quantities spanning $[-10^{-3},\, 10^{-3}]$. The following derivation uses $\advecCoeff$ for notational convenience, but both parameters employ the same transformation. Since a purely logarithmic mapping is undefined near zero, a piecewise transformation is used: a linear core region of half-width $\advecCoeff_0 = 10^{-6}$ and a logarithmic outer region extending to $|\advecCoeff| = \advecCoeff_1 = 10^{-3}$. The normalised variable $\tilde{\advecCoeff} \in [0, 1]$ is related to an intermediate variable $v = 2\tilde{\advecCoeff} - 1 \in [-1, 1]$ \cite{webber2013bi}. The forward transformation from $v$ to $\advecCoeff$ is
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=5.5in]{figures/sim_data_sample.png}
...
Gaussian noise is added to the sensor measurements to simulate real-world measurement error. Two noise levels are considered: a standard deviation of 0.001\% and a standard deviation of 0.01\%. The lower value matches the noise floor recorded by the real sensors. The higher value represents the highest recorded noise in the real-world data and is potential failure mode in certain cases. 

\begin{figure}[H]
    \centering
    \includegraphics[width=6.5in]{figures/synth_col_measNoise.png}
...
% Removed this for now since Kevin recommended it 
% \begin{figure}[H]
%     \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=6in]{figures/synth_col_parNoise.png}
...
This variation examines only the effect of increasing measurement frequency; decreased frequency is instead addressed by the sensor dropout experiments in Section~\ref{sec:exp_set_sensor_dropout}. The baseline measurement interval is 12 hours, consistent with the real-world data. A reduced interval of 4 hours is tested to determine whether the additional temporal resolution improves spatial reconstruction quality, despite the corresponding reduction in the number of raw 30-minute readings averaged into each measurement. Intervals of up to one week were also tested, but produced results that overlapped substantially with the sensor dropout experiments and were not pursued further.

\begin{figure}[H]
    \centering
    \includegraphics[height=4in]{figures/synth_col_measFreq.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=6in]{figures/synth_col_sens_dropout.png}
...
IES is the last of the parameter estimation methods. It maintains an ensemble of parameter vectors, uses their covariance structure to guide updates, and iterates toward the minimum temporal error. Given that 5 pilot points is consistently the optimal number in the preceding sections, it is also fixed to 5 below for comparative validity. Instead, the grid search focuses on the number of iterations and ensemble size, testing $\numIterations \in \{5, 10, 15, 20\}$ and ensemble sizes of $\{50, 100, 150, 200\}$.
