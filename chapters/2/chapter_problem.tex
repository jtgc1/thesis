...
\begin{equation}
   \label{eq:enkf_M}
    \hat{\param}_\nSampTime^{(k)} = M(\hat{\param}_{0}^{(k)}, w_\timevar), 
    \qquad w_\timevar \sim \mathcal{N}_n(0, Q) 
\end{equation}
\vspace{-1em}
\begin{equation} 
   \label{eq:enkf_H}
    \est = H(\hat{\param}_\nSampTime^{(k)}, v_\timevar), 
