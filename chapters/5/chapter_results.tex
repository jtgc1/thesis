...
The real-world results are more compressed than the synthetic ones, reflecting the smaller absolute changes in PHC concentration observed in the field. Unlike the clear separation seen in the synthetic base case, the five methods now occupy substantially overlapping ranges, with median MSE values falling at 1.076 (IDW), 1.1196 (Kriging), 1.219 (4D-Var), 0.9893 (GLM), and 1.016 (IES). GLM and IES retain a modest edge over the distance-based methods, with GLM achieving the lowest mean and median MSE overall and IES the narrowest interquartile range of the parameter estimation methods. However, 4D-Var no longer outperforms the distance-based methods on this dataset, recording both a higher mean and a higher median MSE than IDW and Kriging, a reversal of the trend observed throughout the synthetic evaluation.
